...
OUTPUT_PDF="${1:-$BUILD_DIR/gov-foremoz-whitepaper.pdf}"
...
if [[ ! -x "$PDF_ENGINE" ]]; then
  ...
  echo "  PDF_ENGINE=/Library/TeX/texbin/xelatex ./paper/gov/build.sh" >&2
  exit 1
fi
...
build_pdf "$OUTPUT_PDF" "Foremoz Gov Whitepaper" "${files[@]}"
